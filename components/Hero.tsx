...
import { Volume2, VolumeX, Maximize2, ArrowLeft, Sun, Moon, AlertCircle } from 'lucide-react';
...
const Hero: React.FC<HeroProps> = ({ isDark, toggleTheme }) => {
  ...
  const handleVideoError = (e: React.SyntheticEvent<HTMLVideoElement, Event>) => {
    console.warn("Video playback failed, falling back to poster.", e.currentTarget.error);
    setHasError(true);
  };
    ...
    <div className="relative w-full h-[60vh] overflow-hidden bg-black">
      ...
      <div className="absolute inset-0">
        {!hasError ? (
          ...
          >
            <source src={PROJECT_DATA.hero.videoUrl} type="video/mp4" />
            {/* Fallback for browsers that don't support video tag */}
            <img src={PROJECT_DATA.hero.mediaUrl} alt="Poster" className="w-full h-full object-cover" />
          </video>
        ) : (
          /* Error Fallback State: Show Poster Image */
          <div className="relative w-full h-full">
            <img 
                ...
                className="w-full h-full object-cover" 
            />
            {/* Optional debug indicator - helpful for development */}
            <div className="absolute bottom-24 right-6 bg-black/50 text-white text-[10px] px-2 py-1 rounded backdrop-blur-sm flex items-center gap-1">
      ...
      <div className="absolute bottom-16 left-0 right-0 px-6 z-20 flex flex-col gap-3">
        <div className="flex justify-between items-end text-white/90">
            <span className="text-xs font-medium tracking-wider text-slate-700 dark:text-white/90 transition-colors drop-shadow-md">
                {hasError ? 'PREVIEW IMAGE' : 'HIGHLIGHT REEL'}
            </span>
            
            {/* Only show video controls if video is playing successfully */}
            {!hasError && (
        ...
        <div className="h-1.5 w-full bg-slate-200/30 dark:bg-white/30 rounded-full overflow-hidden backdrop-blur-sm">
            <div className={`h-full bg-brand-pink w-[35%] rounded-full relative ${!hasError ? 'animate-pulse' : ''}`}>
                {!hasError && <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-md scale-100"></div>}
            </div>
        </div>
      </div>
    </div>
